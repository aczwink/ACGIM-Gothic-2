...
//*******************************************************
INSTANCE PC_Item_Potion_BlueMushroom (C_INFO)
{
	nr       		= 4;
	npc				= PC_Hero;
	condition		= PC_Item_Potion_BlueMushroom_Condition;
	information		= PC_Item_Potion_BlueMushroom_Info;
	permanent		= TRUE;
	description		= "Blaupilztrank (2 Feldknöterich, 5 Blauflieder, 5 Dunkelpilz)"; 
};

FUNC INT PC_Item_Potion_BlueMushroom_Condition()
{	
	if((PLAYER_MOBSI_PRODUCTION	 == MOBSI_POTIONALCHEMY) 
...
FUNC VOID PC_Item_Potion_BlueMushroom_Info ()
{
	if (Npc_HasItems (hero, ItPl_Blueplant)    >= 5)
	&& (Npc_HasItems (hero, ItPl_Temp_Herb) 	   >= 2)
	&& (Npc_HasItems (hero, ItPl_Mushroom_01) 	   >= 5)
	{
		Npc_RemoveInvItems (hero,ItPl_Blueplant, 5);
		Npc_RemoveInvItems (hero,ItPl_Mushroom_01, 5);
		Npc_RemoveInvItems (hero,ItPl_Temp_Herb, 2);
		
		CreateInvItems(hero,Item_Potion_BlueMushroom,1);  
		Print (PRINT_AlchemySuccess);
	}
		else 
	{
