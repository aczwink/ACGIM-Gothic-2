...
func void DIA_Pyrokar_OATH_Info ()
{
	...
	CreateInvItems(hero, Item_Armor_AdeptRobe, 1);
	AI_EquipArmor(hero, Item_Armor_AdeptRobe);
...
FUNC VOID DIA_Pyrokar_Wunsch_Dyrian ()
{
	...
	B_StartOtherRoutine (Nabiz,"FAVOUR");
...
///Babo soll Klostergarten übernehmen
instance DIA_Pyrokar_Klostergarten		(C_INFO)
{
	npc			 = 	KDF_500_Pyrokar;
	nr			 = 	2;
	condition	 = 	DIA_Pyrokar_Klostergarten_Condition;
	information	 = 	DIA_Pyrokar_Klostergarten_Babo;
	PERMANENT	 =  FALSE;
	description	 = 	"Der Novize Babo soll die Leitung des Klostergartens übernehmen.";
};
func int DIA_Pyrokar_Klostergarten_Condition ()
{	
	if (other.guild == GIL_KDF)
	&& (Kapitel < 2)
	{
...
FUNC VOID DIA_Pyrokar_Klostergarten_Babo ()
{
	//AC: hier sollte der dialog etwas sinnvoller sein. Argumentation: Agon kümmert sich nicht mehr um die Pflanzen (ist weg oder tot)
	//da der spieler jetzt magier ist kann er das vorschlagen
	...
	Info_ClearChoices (DIA_Pyrokar_Klostergarten);
};
	...
	if (hero.guild == GIL_KDF)
		{
			...
			AI_Output (self, other, "DIA_PYROKAR_SJCGIMCUT_01"); //Viel Zeit ist vergangen, seit du den Bund mit dem Feuer eingegangen bist.
			AI_Output (self, other, "DIA_Pyrokar_OATH_11_07"); //Trage diese Robe als Zeichen des ewigen Bundes.
			...
			hero.attribute[ATR_REGENERATEHP] = 2147483647; //mana regeneration does not work without life regeneration-.- (so regenerate as few life as possible)
			hero.attribute[ATR_REGENERATEMANA] = 1; //hero.attribute[ATR_REGENERATEMANA] = x, x seconds until 1 point is regenerated
			AI_PrintScreen("Pyrokar hat dich gelehrt", -1, 40, FONT_Screen, 5);
			AI_PrintScreen("magische Kraft aus dir selbst zu ziehen", -1, 45, FONT_Screen, 5);
